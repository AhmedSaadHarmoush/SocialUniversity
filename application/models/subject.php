...
class subject extends CI_Model {
    ...
    public function getSubjectsJoins($user_id,$dep_id){
        ...
        if($query->num_rows>=0){
            ...
            return $this->sperate2($filtered,$second);
        }
        else {
            return FALSE;
        }
   ...
   public function dunregister($data) {
    ...
    public function sperate2 ($first , $second) {
        if (empty($second) || empty($first)) {
            return $first;
        }
        $result = array();
        foreach ($first as $subject1) {
            $flag=0;
            foreach ($second as $subject2) {
                if ($subject2['subject_id']==$subject1['id']) {
                    $flag=1;
                    break;
                }
            }
